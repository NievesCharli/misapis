...
export const getAllEjemplos = async (req, res) => {
    ...
    try {
       ...
       return res.status(500).json({ message: 'Error al obtener los ejemplos' });
    }
};

export const getEjemploById = async (req, res) => {
    console.log("getEjemploById");
    ...
    try {
        ...
        return res.status(500).json({ message: 'Error al obtener el ejemplo' });
    }
};

export const postEjemplo = async (req, res) => {
    console.log("postEjemplo");
    const body = req.body;
    ...
    try {
        ...
        return res.status(500).json({ message: 'Error al crear el ejemplo' });
    }
};

export const putEjemplo = async (req, res) => {
    console.log("putEjemplo");
    const id = req.params.id;
    const body = req.body;
    try {
        if(!mongoose.Types.ObjectId.isValid(id)){
        ...
        return res.status(500).json({ message: 'Error al actualizar el ejemplo' });
    }
};

export const deleteEjemplo = async (req, res) => {
    console.log("deleteEjemplo");
    const id = req.params.id;
    try {
        if(!mongoose.Types.ObjectId.isValid(id)){
            return res.status(400).json({ message: 'ID no válido' });
        ...
        return res.status(200).json({ message: 'Ejemplo eliminado correctamente', ejemplo });
    } catch (error) {
        return res.status(500).json({ message: 'Error al eliminar el ejemplo' });
    }
};